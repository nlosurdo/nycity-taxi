...
taxi_common_data = taxi_common_data.withColumn("pickup_day_of_week", F.dayofweek("pickup_datetime"))
taxi_common_data = taxi_common_data.withColumn("pickup_hour", F.hour("pickup_datetime"))
taxi_common_data = taxi_common_data.withColumn("pickup_minute", F.minute("pickup_datetime"))
...
taxi_common_data_cl = cluster_coordinates(taxi_common_data, "pickup_latitude", "pickup_longitude", 100, "pickup_clusters_m")
taxi_common_data_cl = cluster_coordinates(taxi_common_data_cl, "dropoff_latitude", "dropoff_longitude", 100, "dropoff_clusters_m")

# Apply clustering for pickup and dropoff with 200 clusters (large granularity)
taxi_common_data_cl = cluster_coordinates(taxi_common_data_cl, "pickup_latitude", "pickup_longitude", 200, "pickup_clusters_l")
taxi_common_data_cl = cluster_coordinates(taxi_common_data_cl, "dropoff_latitude", "dropoff_longitude", 200, "dropoff_clusters_l")
...
taxi_common_data_cl.write.mode("overwrite").saveAsTable("hive_metastore.nycity_taxi.taxi_processed_data")

# COMMAND ----------

# Step 6: Remove unnecessary columns
columns_to_remove = ["trip_time_in_secs", "fare_amount", "tip_amount", "tolls_amount", "pickup_datetime", "dropoff_datetime",
...
taxi_processed_data_pandas = taxi_processed_data.select('pickup_latitude', 'pickup_longitude', 'pickup_clusters_m').limit(30000).toPandas()

# Proceed with your Folium map creation
